...
from src.transforms import reshape_fn, MultiScaleTrans, reshape_fn2
...
class COCOYoloDataset:
    ...
    def __getitem__(self, index):
        ...
        for bbox, label in zip(bboxes, labels):
            ...
            # cv2.rectangle(img_cv2, (int(bbox[0]),int(bbox[1])), (int(bbox[2]),int(bbox[3])), (255,0,0))
            tmp.extend(bbox)
            tmp.append(int(label))
            # tmp [x_min y_min x_max y_max, label]
            out_target.append(tmp)
        # cv2.imwrite('temp.jpg', img_cv2)
...
def create_yolo_dataset(image_dir, anno_path, batch_size, max_epoch, device_num, rank,
                        config=None, is_training=True, shuffle=True, corrupt_aug=False):
    ...
    num_parallel_workers = min(batch_size, int(cores / device_num))
    if is_training:
        multi_scale_trans = MultiScaleTrans(config, device_num, corrupt_aug=corrupt_aug)
...
def create_yolo_datasetv2(image_dir,
    ...
    yolo_dataset = COCOYoloDatasetv2(root=image_dir)
    distributed_sampler = DistributedSampler(len(yolo_dataset), device_num, rank, shuffle=shuffle)
    hwc_to_chw = CV.HWC2CHW()

    config.dataset_size = len(yolo_dataset)

    ds = de.GeneratorDataset(yolo_dataset, column_names=["image", "img_id", "ori_image"],
                             sampler=distributed_sampler)
    compose_map_func = (lambda image, img_id, ori_image: reshape_fn2(image, img_id, config, ori_image))
    ds = ds.map(input_columns=["image", "img_id", "ori_image"],
                output_columns=["image", "image_shape", "img_id", "ori_image"],
                column_order=["image", "image_shape", "img_id", "ori_image"],
                operations=compose_map_func, num_parallel_workers=1)
    ds = ds.map(input_columns=["image"], operations=hwc_to_chw, num_parallel_workers=1)
    ds = ds.batch(batch_size, drop_remainder=True)
    ds = ds.repeat(max_epoch)
    return ds, len(yolo_dataset)
