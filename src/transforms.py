...
def _reshape_data(image, image_size):
    ...
    image, _, _ = letterbox(image, (h, w))
    # cv2.imwrite('letterbox.jpg',image)
...
def _data_aug(image, box, jitter, hue, sat, val, image_input_size, max_boxes,
              anchors, num_classes, max_trial=10, device_num=1, corrupt_aug=False):
    ...
    if _rand() < 0.7:
        ...
        box_data = box
        image = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
        image, ratio, pad = letterbox(image, (input_h, input_w), auto=False)
        box_data[:, 0] = ratio[0] * box_data[:, 0] + pad[0]  # pad width
        box_data[:, 1] = ratio[1] * box_data[:, 1] + pad[1]  # pad height
        box_data[:, 2] = ratio[0] * box_data[:, 2] + pad[0]
        box_data[:, 3] = ratio[1] * box_data[:, 3] + pad[1]

    if corrupt_aug and _rand() < 0.9:
        image = ImageCorrupt(image)
    else:
...
def preprocess_fn(image, box, config, input_size, device_num, corrupt_aug=False):
    ...
    image, anno = _data_aug(image, box, jitter=jitter, hue=hue, sat=sat, val=val,
                            ...
                            num_classes=num_classes, anchors=anchors, device_num=device_num, corrupt_aug=corrupt_aug)
    return image, anno
...
def reshape_fn2(image, img_id, config, ori_image):
    input_size = config.test_img_shape
    image, ori_image_shape = _reshape_data(image, image_size=input_size)
    return image, ori_image_shape, img_id, ori_image


class MultiScaleTrans:
    """Multi scale transform."""
    def __init__(self, config, device_num, corrupt_aug=False):
        self.config = config
        self.seed = 0
        self.size_list = []
    ...
    def __call__(self, imgs, annos, x1, x2, x3, x4, x5, x6, batchInfo):
        ...
        for img, anno in zip(imgs, annos):
            img, anno = preprocess_fn(img, anno, self.config, input_size, self.device_num, corrupt_aug=self.corrupt_aug)
